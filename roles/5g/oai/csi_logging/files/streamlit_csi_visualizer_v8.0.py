#!/usr/bin/env python3
"""
CSI Visualizer v8.0 - VECTORIZED & ADAPTIVE
...
st.set_page_config(page_title="CSI Visualizer v8.0", page_icon="📊", layout="wide", initial_sidebar_state="expanded")
...
# ===================== UI =====================
st.markdown("# 📊 CSI Visualizer v8.0")
st.markdown("**Vectorized · adaptive · time-binned heatmaps · std-based**")
...
with tabs[1]:  st.pyplot(plot_distribution(mags_f), width='stretch')
with tabs[2]:  st.pyplot(plot_per_rb(mags_f, rbs_f), width='stretch')
with tabs[3]:  st.pyplot(plot_heatmap_mag_with_time(mags_f, rbs_f, frames_f, ts_f, ants_f, hm_window, hm_per_ant), width='stretch')
with tabs[4]:  st.pyplot(plot_timeline(mags_f, phases_f, ts_f), width='stretch')
with tabs[5]:  st.pyplot(plot_heatmap_phase_with_time(phases_f, rbs_f, frames_f, ts_f, ants_f, hm_window, hm_per_ant), width='stretch')
with tabs[6]:  st.pyplot(plot_multipath(mags_f, phases_f, rbs_f), width='stretch')
with tabs[7]:  st.pyplot(plot_mimo(mags_f, slots_f, ants_f, ports_f, stats), width='stretch')
with tabs[8]:  st.pyplot(plot_slot_signal_quality(mags_f, slots_f, stats), width='stretch')
with tabs[9]:  st.pyplot(plot_slot_temporal_stability(mags_f, slots_f, frames_f, ts_f, hm_window), width='stretch')
with tabs[10]: st.pyplot(plot_slot_rb_pattern(mags_f, slots_f, rbs_f), width='stretch')
with tabs[11]:
    if len(np.unique(ants_f)) > 1: st.pyplot(plot_antenna_comparison_diag(mags_f, ants_f), width='stretch')
    else: st.info("Only one antenna in filtered data")
with tabs[12]: st.pyplot(plot_rb_noise_profile(mags_f, rbs_f, ants_f), width='stretch')
with tabs[13]: st.pyplot(plot_time_anomalies(mags_f, ts_f), width='stretch')
with tabs[14]: st.pyplot(plot_phase_analysis(phases_f, mags_f), width='stretch')

if gaps:
    st.divider(); st.subheader("⚠️ Gaps Detected"); st.dataframe(pd.DataFrame(gaps), width='stretch')
st.markdown(f"**v8.0** | {len(mags_f):,} records | {'6-col' if len(stats['ant_list'])==1 else '8-col'} | vectorized ✅")

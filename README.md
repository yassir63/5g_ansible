...
- Noise generation done via the first fit node (`interference_usrp=fit`) or a specified USRP (`n300` or `n320`).
- Spectrum visualization using `uhd_fft`, after the fit node used to generate noise if any. 
...
The deployment is driven by `inventory/default/hosts.ini`, which is generated by the *deploy.sh* script:

- Core, RAN, and Monitoring nodes
- Which node acts as Kubernetes master/worker
- R2Lab Faraday jump-host and remote UEs
- RRU to use (or rfsim mode)
...
- You may assign any node for any role (core, ran, monitor) **except** that Benetel RUs can only be deployed on `sopnode-f3`.
- Supported RRUs are: USRP `n300` and `n320`, AW2S `jaguar` and `panther`, RAN550 RUs `benetel1` and `benetel2`.
- The entire `[fit_nodes]` group, as well as the frequency, gain (g) and sample rate / bandwidth covered by the noise generator specified in `[faraday]`, are only relevant to the interference test scenario and are not needed otherwise.
- For the interference scenario, the first fist node (`groups[fit_nodes][0]`) is used for noise generation (`interference_usrp`). The USRP used for spectrum visualization can be `=fit`(meaning the other fit node's USRP will be used as noise generator), or `=n300` / `=n320`).
